...
(function($, sr) {
	...
	jQuery.fn[sr] = function(fn) {
		return;
			...
			fn ? this.bind('resize', debounce(fn)) :
			this.trigger(sr);
	};
})(jQuery, 'smartresize');
/**
...
function init_sidebar() {
	...
	var setContentHeight = function() {
		...
		var bodyHeight = $BODY.outerHeight(),
			footerHeight =
				$BODY.hasClass('footer_fixed') ? -10 :
				$FOOTER.height(),
			leftColHeight = $LEFT_COL.eq(1).height() + $SIDEBAR_FOOTER.height(),
			contentHeight =

					bodyHeight < leftColHeight ? leftColHeight :
					bodyHeight;
...
$.fn.popover.Constructor.prototype.leave = function(obj) {
	var self =
			...
			obj instanceof this.constructor ? obj :
...
function init_echarts() {
	...
	if ($('#salary_pie').length) {
		var company_id = localStorage.getItem('company_id');
		// var pathArray = window.location.pathname.split( '/' );
		var employee_id = $.urlParam('id'); //pathArray[4].replace(/%20/g,' ');
		var echartPie = echarts.init(document.getElementById('salary_pie'), theme);
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.status == '200') {
					...
					// $('#date_of_employment').html(response['data']['designations']['date_of_employment']);
					...
					$('#ttd_title_1').append(' | ₦' + response['data']['gross_total']); //dislay total on title

					if (response.data.gross.length != 0) {
						var list_of_names = [];
						var list_of_values = [];
						$(response.data.gross).each(function(index, value) {
							list_of_names.push(value.name);
							list_of_values.push({ value: Number(value.amount), name: value.name });
						});

						var echartDonut = echarts.init(
							document.getElementById('salary_pie'),
							theme,
						);
	...
	if ($('#echart_pie2').length) {
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.status == '200') {
					$('#ddsh_loading').hide(); //hidel loader
					$('#ttd_title').append(' | ₦' + response['data']['total_deduction']); //dislay total on title

					if (response.data.deductions.length != 0) {
						var list_of_names = [];
						var list_of_values = [];
						$(response.data.deductions).each(function(index, value) {
							list_of_names.push(value.name);
							list_of_values.push({ value: Number(value.amount), name: value.name });
						});
