function SmartWizard(target, options) {
	...
	var _showStep = function($this, stepIdx) {
		...
		if ($this.options.transitionEffect == 'slide') {
			...
			if (stepIdx == prevCurStepIdx) {
				nextElmLeft1 =
					$($(selStep, $this.target).attr('href'), $this.target).outerWidth() + 20;
				nextElmLeft2 = 0;
				curElementLeft =
					0 - $($(curStep, $this.target).attr('href'), $this.target).outerWidth();
			} else {
	...
	var _adjustButton = function($this) {
		...
		if (
			(!$this.steps.hasClass('disabled') &&
				!$this.buttons.previous.hasClass('buttonDisabled')) ||
			$this.options.enableFinishButton
		) {
			$($this.buttons.finish).removeClass('buttonDisabled');
			if ($this.options.hideButtonsOnDisabled) {
				$($this.buttons.finish).show();
				// $($this.buttons.finish).on('click',addProduct);
			}
		} else {
