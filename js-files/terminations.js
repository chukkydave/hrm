		...
		.then(function(response) {
			...
			if (response.data.data.length > 0) {
				$(response.data.data).map((i, v) => {
					term_list += `<tr class="even pointer" id="term_row${v.exited_id}">`;
					...
					if (v.employee_status.toLowerCase() === 'inactive') {
						...
						<div class="dropdown">
							...
							<ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
								<li onClick="viewterm(${v.exited_id})">
				...
				$('#list_term_body').html(`<tr><td colspan="6">No record</td></tr>`);
				$('#list_term_loader').hide();
				$('#list_term_table').show();
			}
		})
...
function updateSchedule() {
	...
	if (comment.length < 1) {
		...
		let upImage = document.querySelector(`#sche_msg_file`).files[0];
...
function viewSchedule(exit_id, employee_id) {
	...
	axios
		...
		.then(function(response) {
			let { exit_document, exit_note } = response.data.data;
