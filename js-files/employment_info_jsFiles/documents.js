...
function viewDocument(id) {
	...
	axios
		...
		.then(function(response) {
			...
			let start = moment(date_uploaded, 'YYYY-MM-DD HH:mm:ss').format('LL');
			$('#edit_docx_name').html(document_name);
			$('#edit_docx_type').html(type);
			$('#edit_docx_date').html(start);
		...
		.catch(function(error) {
			...
			$('#edit_docx_error').html(error.statusText);
		})
		.then(function() {
			// always executed
		});
...
function list_doctype() {
	...
	$.ajax({
		...
		error: function(response) {
			options += `<option>Error loading</option>`;
